...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Masuk' (Login) button on the homepage to open the login form.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div/nav/div[3]/a').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the username (user@glunova.id) into the email field, then type the password (user123), then click the 'Masuk Dashboard' button.
        frame = context.pages[-1]
        # Input text
        ...
        # -> Click the 'Roadmap' navigation item to open the Roadmap page (anchor at index 1534).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div/nav/div[2]/div/a[2]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Click the first activity's completion button (Jalan Santai Pagi) to mark it done, then verify the UI updates to show 'Selesai!', updated progress percentage, and any milestone unlock state.
        frame = context.pages[-1]
        # Click element
