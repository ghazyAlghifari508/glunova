...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Masuk' (Login) button to navigate to the login page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/main/div/div/nav/div[3]/a/button').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Navigate to http://localhost:3000/login (explicit test instruction).
        await page.goto("http://localhost:3000/login")
        
        # -> Type 'valid.user@example.com' into the email field (index 1493). ASSERTION: Will input the test email into the email field.
        frame = context.pages[-1]
        # Input text
